    ...
    : QDeviceDiscovery(types, parent)
{
    ...
    if (m_types & Device_VideoMask) {
        m_fileWatcher->addPath(QString::fromLatin1(QT_DRM_DEVICE_PATH));// "/dev/dri/"
    }
...
void QDeviceDiscoveryHotPlug::handleHotPlugWatch(const QString &path)
{
    ...
    // Note, the equipment obtained here has been filtered, because when the class is instantiated
    // Filter parameters have been passed in, such as: QDeviceDiscovery::Device_Keyboard
